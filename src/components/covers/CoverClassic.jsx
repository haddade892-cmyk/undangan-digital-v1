...
function CornerFrame({ position }) {
  ...
  return (
    <svg width="60" height="60" viewBox="0 0 60 60" className={`absolute top-6 ${isRight ? 'right-6 scale-x-[-1]' : 'left-6'} opacity-60`}>
...
function CoverClassic({ onOpen }) {
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-center text-white px-6 text-center overflow-hidden">
      <CoverBackground />
      <div className="absolute inset-0" style={{ background: 'radial-gradient(ellipse at center, rgba(0,0,0,0.35) 0%, rgba(0,0,0,0.65) 100%)' }} />
      <CornerFrame position="left" />
      <CornerFrame position="right" />
      <div className="relative z-10 transition-all duration-1000" style={{ opacity: visible ? 1 : 0, transform: visible ? 'scale(1)' : 'scale(1.04)' }}>
        ...
        <button onClick={onOpen} className="border border-white/60 rounded-full px-8 py-3 text-xs tracking-[0.2em] uppercase hover:bg-white/10 transition">
          Buka Undangan
        </button>
      </div>
    </div>
